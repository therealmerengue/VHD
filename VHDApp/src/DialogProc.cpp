...
namespace DialogProc
{
	...
	INT_PTR CALLBACK ChooseDiskDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_INITDIALOG:
			...
			Combobox::AddItemsToCombobox(hwndCombo, GetDriveLetters());
			EnumChildWindows(hwndDlg, (WNDENUMPROC)SetFont, (LPARAM)GetStockObject(DEFAULT_GUI_FONT));
			break;
		...
		case WM_COMMAND:
			...
			{
				hwndCombo = GetDlgItem(hwndDlg, ID_COMBO_DISK);
				string diskPath = Combobox::TextFromComboboxToString(hwndCombo);
		...
		case WM_COMMAND:
			...
			if (LOWORD(wParam == ID_BTN_DECRYPT))
			{
				if (!hwndTV)
				{
					...
					std::chrono::steady_clock::time_point end_time = std::chrono::steady_clock::now();
					Dialog::ShowTimeDialog(hwndDlg, begin_time, end_time, L"Decrypted in: ");
